...
import io.cdap.plugin.kafka.connector.KafkaConnectorConfig;
...
public class KafkaBatchSource extends BatchSource<KafkaKey, KafkaMessage, StructuredRecord> {
  ...
  public static class Kafka10BatchConfig extends KafkaBatchConfig {
    ...
    private KafkaConnectorConfig connection;

    public Kafka10BatchConfig() {
      super();
    }

    public Kafka10BatchConfig(String partitions, String topic, String initialPartitionOffsets) {
      super(partitions, topic, initialPartitionOffsets);
    }

    public String getBrokers() {
      return connection.getKafkaBrokers();
    }
  ...
  public void transform(KeyValue<KafkaKey, KafkaMessage> input, Emitter<StructuredRecord> emitter) {
    ...
    if (config.getFormat() == null) {
      builder.set(messageField, getByteArray(input.getValue().getPayload()));
    } else {
  ...
  private static byte[] getByteArray(ByteBuffer buffer) {
    // return underlying byte array if available.
    if (buffer.hasArray()) {
      return buffer.array();
    }

    // Copy buffer contents into new array and return the array.
